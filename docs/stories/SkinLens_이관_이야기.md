...
> **이삿짐 주의(보안).** `migrate_export.sh`가 만드는 번들(tgz)에는 **`.env`(비밀)+DB덤프**가 들어갑니다. 반드시 **전송 암호화 + 사용 후 안전 삭제**하고, 비밀 포함 tgz를 디스크에 방치하지 마세요(감사 `../architecture/01_SkinLens_운영아키텍처_최종리뷰.md ②-E`).
