...
> 심화편은 지금 **구축·하드닝·이관·배포** 네 편이 있고(막 1·2·5·6), 운영·백업은 아래 각 막의 요약과
> 막 끝의 "더 자세히 →" 링크로 갈음합니다.
...
세부 배포 등장인물(공방=서비스 경로, 집사=러너, **이미지 창고**=GHCR, 지배인=`deploy.sh` …)은 「SkinLens 배포 이야기」에 있고, 세 편 공용 매핑은 `./README.md`(비유 사전)에 모여 있습니다.
...
flowchart LR
  ...
  TICKET --> QUEUE["③ 접수 대장<br/>Job Queue(Postgres)<br/>redis=빠른 신호(Phase 3)"]
...
> 이 막을 더 자세히 — 골조·SSH 열쇠·다중 계정·조리대·네트워킹·준공 검사까지는 **「SkinLens 구축 이야기」**(`./SkinLens_구축_이야기.md`)에서 단계별로 다룹니다.
...
>
> 이 막을 단계별로 더 자세히 → **「SkinLens 하드닝 이야기」**(`./SkinLens_하드닝_이야기.md`).
...
> 더 자세히 → **`../server-setup/windows11_ubuntu_server_setup.md`**(부팅 기동·`hwclock` 시계·healthcheck·로그 로테이션·자동 업데이트/재부팅 정책). 운영 점검 스크립트는 `../../deploy/scripts/verify_ops.sh`.
...
>
> 더 자세히 → **`../../deploy/scripts/`의 `pg_backup.sh`·`wsl-backup-task.ps1`**(오프사이트·암호화·보존 하한·데드맨 스위치). 운영 원본/PII 정리는 `../../deploy/ops-jobs/retention.py`.
...
>
> 이 막을 단계별로 더 자세히 → **「SkinLens 이관 이야기」**(`./SkinLens_이관_이야기.md`).
