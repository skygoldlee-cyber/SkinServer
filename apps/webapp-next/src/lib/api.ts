...
const BASE = process.env.NEXT_PUBLIC_AI_API_BASE ?? "/api";

// 고객 자가보고 설문 — packages/common/skinlens_contract 의 Survey shape 와 일치.
// 앱은 아는 필드만 채워 본내고, 엔진은 아는 필드만 사용한다(모두 선택).
...
// 세션 만료/로그아웃 상태에서 무인증 헤더로 진행하면 원인 불명의 401 이 되므로,
// 토큰 부재 시 즉시 명확한 에러를 던져 실패를 빠르게 표면화한다(N2).
...
export async function analyze(image: File, survey?: Survey): Promise<{ job_id: string }> {
  ...
  // presign 이 박아둔 job_id 를 함께 본내 스토리지 키와 DB 잡을 결합한다(N1).
  const res = await fetch(`${BASE}/analyze`, {
    method: "POST",
    headers: { ...headers, "Content-Type": "application/json" },
    body: JSON.stringify({ image_key: pre.image_key, job_id: pre.job_id, survey }),
  });
