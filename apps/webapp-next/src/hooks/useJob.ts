...
//
// 지수 백오프: 고정 간격 폴리는 모바일/저전력 환경에서 배터리·데이터를 소모한다.
// 성공할 때마다 간격을 intervalMs 부터 최대 maxIntervalMs 까지 지수적으로 늘리고,
// 상태가 바뀌면(=진전이 있으면) intervalMs 로 리셋한다. 탭이 백그라운드로 가면
// (visibilitychange) 폴리를 잠시 멈춰 소모를 줄인다.
